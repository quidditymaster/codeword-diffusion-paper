...
Either of $\epsilon^t$ or $x^0$ may then be used as a prediction target for a denoising model.

\subsubsection{Denoising Model}

A denoising model $f(x^t, t, \theta)$ can be trained using samples of ($x^t$, $\epsilon^t$). The specific meaning of the approximation in equation \ref{denoiseapprox} can be made explicit by a choice of loss function to minimize, for this work we use binary cross entropy. 
...
We create generative models which are trained on a dataset consisting of examples of valid solved sudoku puzzles represented as a collection of human readable texts. 
